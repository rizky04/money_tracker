<div class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-8 py-3 z-50 flex justify-between items-center shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
    ...
    <a href="{{ route('history') }}" class="flex flex-col items-center gap-1 {{ request()->routeIs('history') ? 'text-gas-green font-bold' : 'text-gray-400' }}">
        <svg class="w-6 h-6" fill="{{ request()->routeIs('history') ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"></path>
        </svg>
        <span class="text-[10px]">Riwayat</span>
    </a>

    <!-- Tombol AI Assistant (diganti posisi dengan kamera) -->
    <a href="{{ route('chatbot.index') }}" class="flex flex-col items-center gap-1 {{ request()->routeIs('chatbot.*') ? 'text-gas-green font-bold' : 'text-gray-400' }}">
        <div class="relative -top-6" x-data="{}">
            <div class="bg-gas-black text-white p-4 rounded-full shadow-lg border-4 border-gray-100 active:scale-95 transition-transform">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
                </svg>
            </div>
        </div>
        <span class="text-[10px] -mt-5">AI</span>
    </a>

    <a href="{{ route('stats') }}" class="flex flex-col items-center gap-1 {{ request()->routeIs('stats') ? 'text-gas-green font-bold' : 'text-gray-400' }}">
        <svg class="w-6 h-6" fill="{{ request()->routeIs('stats') ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
