<div class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-8 py-3 z-50 flex justify-between items-center shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
    ...
    <a href="{{ route('history') }}" class="flex flex-col items-center gap-1 {{ request()->routeIs('history') ? 'text-gas-green font-bold' : 'text-gray-400' }}">
        <svg class="w-6 h-6" fill="{{ request()->routeIs('history') ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"></path></svg>
        <span class="text-[10px]">Riwayat</span>
    </a>



<div class="relative -top-6" x-data="{}">
    <button
        type="button"
        @click="$dispatch('buka-kamera')"
        class="bg-gas-black text-white p-4 rounded-full shadow-lg border-4 border-gray-100 active:scale-95 transition-transform">
    ...
    <a href="{{ route('account') }}" class="flex flex-col items-center gap-1 {{ request()->routeIs('account') ? 'text-gas-green font-bold' : 'text-gray-400' }}">
        <svg class="w-6 h-6" fill="{{ request()->routeIs('account') ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
